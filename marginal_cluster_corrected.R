...
# 
# for marginal cluster-level test WITH finite-sample consideration
##########################################################################################

marginal.cluster_corrected <- function(choice, parameter, nsims=5000){
  
  #number of iterations
  nsims <- nsims
  
  #mean of unequal cluster sizes
  m_bar <- parameter[1]
  #outcome ICC
  rho <- parameter[2]
  ...
  if (choice==1){ #TURE PARAMETER SET FOR COMPUTING EMPIRICAL POWER
    #set assumed parameter estimates for generating data
    beta1 <- 1
    beta2 <- 0.15
    beta3 <- 0.05
    beta4 <- 2*(delta_x-beta2)
  } else if (choice==2){ #TURE PARAMETER SET FOR COMPUTING EMPIRICAL TYPE I ERROR
    beta1 <- 1
    beta2 <- 0.15
    beta3 <- 0.05
    beta4 <- -2*beta2
  ...
  for (j in 1:nsims){
    ...
    test.stat <- as.numeric((t(R)%*%beta)/sqrt(t(R)%*%vcov(fit)%*%R))
    p[j] <- 1-pf(test.stat^2,1,n-2)
  }
