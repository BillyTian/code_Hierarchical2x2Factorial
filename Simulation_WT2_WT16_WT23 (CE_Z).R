...
# Web Table 2,  Web Table 16, and Web Table 23
...
for (i in 1:nrow(table)){
  ...
  n[i] <- n_calc_beta3(eff=delta, m_bar=m_bar, rho=rho, CV=CV)
  pred.power[i] <- power_calc_beta3(n=n[i], eff=delta, m_bar=m_bar, rho=rho, CV=CV)
}
...
empirical_beta3 <- function(nullcase=F, parameter, nsims=5000){
...
for (i in 1:nrow(table)){
  empirical.power <- rbind(empirical.power, empirical_beta3(parameter=table[i,]))
}

#Compute empirical type I error (and error rate)
empirical.tIe <- NULL
for (i in 1:nrow(table)){
  empirical.tIe <- rbind(empirical.tIe, empirical_beta3(nullcase=T, parameter=table[i,]))
}
...
mainDir = '...'
write.table(result, paste0(mainDir, "/CE_Z_", delta, ".csv"), sep =",", row.names=F)
