...
# 
# for marginal cluster-level test WITHOUT finite-sample consideration
##########################################################################################

marginal.cluster <- function(choice, parameter, nsims=5000){
  ...
  if (choice==1){ #TRUE PARAMETER SET FOR COMPUTING EMPIRICAL POWER
    #set assumed parameter estimates for generating data
    beta1 <- 1
    beta2 <- 0.15
    beta3 <- 0.05
    beta4 <- 2*(delta_x-beta2)
  } else if (choice==2){ #TRUE PARAMETER SET FOR COMPUTING EMPIRICAL TYPE I ERROR
    beta1 <- 1
    beta2 <- 0.15
    beta3 <- 0.05
    beta4 <- -2*beta2
